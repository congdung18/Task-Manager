...
export const queryBuilder = asyncWrapper(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    const paginationData = matchedData(req.query) as {
        page: number,
        limit: number
    }

    req.pagination.page = paginationData.page;
    req.pagination.limit = paginationData.limit;
    req.pagination.skip = (paginationData.page - 1) * paginationData.limit;

    if (req.query.status){
        const validSort = ["pending", "in-progress", "done", "overdue"];
        
        const statusData = matchedData(req.query) as {
            status: string;
        }

        if (!validSort.includes(statusData.status)){
            return next(new ValidationError("task"));
        }
    }
})
